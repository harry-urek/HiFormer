...
from utils import DiceLoss, test_single_volume, get_latest_checkpoint, save_checkpoint
...
def trainer(args, model, snapshot_path):
    ...
    # Save logs
    logging.basicConfig(
        filename=f"{snapshot_path}/{args.model_name}{date_and_time}_log.txt",
        level=logging.INFO,
    ...
    db_test = Synapse_dataset(base_dir=args.test_path,
    ...
    for epoch_num in iterator:
        ...
        if epoch_num >= max_epoch - 1:
            ...
            if (epoch_num + 1) % args.eval_interval != 0:
                logging.info("*" * 20)
                logging.info(
                    f"Running Inference after epoch {epoch_num} (Last Epoch)")
